...
<html lang="en">
<head>
    <title>Make User (Admin) : CCNB</title>
    <meta charset="utf-8">
        ...
        if (isset($_SESSION['departId'])) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                if ($userlevel == 1) {
                    ...
                    $query1 = "UPDATE login_details SET userlevel=NULL WHERE departmentId='$departmentID'";
                    $result1 = mysqli_query($mysql, $query1) or die ("<h3>Sorry! Couldn\'t connect!.2</h3>");

                    echo "<h3 style='text-align: center; color: #2daae4; margin-bottom: 10px'>Dear <span style='color: dodgerblue'>" . $_SESSION['name'] . "</span>, <span style='color: dodgerblue'>" . $name . "</span> is a <span style='color: dodgerblue'>User</span> now!</h3>";
                    echo '<div class="form">
                                            <a href="make_admin.php" class="form2 btn btn-danger btn-block">Make Admin</a>
                                        </div>
                                        <form action="edit_others_user.php" method="post">
                                            <button class="form1 btn btn-warning btn-block">Back</button>
                                        </form>';
                } else {
                    echo "<h3 style='text-align: center; color: #2daae4; margin-bottom: 10px'>Dear <span style='color: dodgerblue'>" . $_SESSION['name'] . "</span>, <span style='color: dodgerblue'>" . $name . "</span> is already a <span style='color: dodgerblue'>User</span>!</h3>";
                    echo '<div class="form">
                                            <a href="make_admin.php" class="form2 btn btn-danger btn-block">Make Admin</a>
                                        </div>
                                        <form action="edit_others_user.php" method="post">
