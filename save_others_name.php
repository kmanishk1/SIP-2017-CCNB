...
<html lang="en">
<head>
    <title>Save Others' Profile Name (Admin) : CCNB</title>
    <meta charset="utf-8">
...
<body>
    <?php
        ...
        if (isset($_POST['name']) AND isset($_POST['password']) AND isset($_SESSION['departId'])) {
            ...
            if ($password === $pass) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $query1 = "UPDATE login_details SET name='$newname' WHERE departmentId='$departmentID'";
                    $result1 = mysqli_query($mysql, $query1) or die ("<form action=\"edit_others_name.php\" method=\"post\"><button class=\"form1 btn btn-success btn-block\">Back</button></form>");

                    echo "<h3 style='text-align: center; color: #2daae4; margin-bottom: 10px'>Dear <span style='color: dodgerblue'>" . $_SESSION['name'] . "</span>, you've updated <span style='color: dodgerblue'>".$name."</span>'s Profile Name successfully to <span style='color: dodgerblue'>".$newname."</span>!</h3>";
                    echo '
                                                        <div class="form">
